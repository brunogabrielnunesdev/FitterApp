...
public class ProfileMapper {
    ...
    public CreateProfileCommand toCreateCommand(
            UUID userId,
    ...
    public UpdateProfileDraftCommand toUpdateCommand(
            UUID userId,
            UUID profileId,
            UpdateProfileDraftRequestDto request) {
    ...
    public UpdateProfileServiceModesCommand toServiceModesCommand(
            UUID userId,
            UUID profileId,
            UpdateServiceModesRequestDto request) {
    ...
    public UpdateProfileServiceAreasCommand toServiceAreasCommand(
            UUID userId,
            UUID profileId,
            UpdateServiceAreasRequestDto request) {
        List<ServiceAreaInput> serviceAreas = request.serviceAreas().stream()
                .map(this::toServiceAreaInput)
                .toList();
    ...
    public ProfileStatusResponseDto toStatusResponse(com.fitterapp.personal.entity.profile.Profile profile) {
        var revision = profile.getCurrentRevision();
        return new ProfileStatusResponseDto(profile.getId(), profile.getFullName(), profile.getStatus(),
                revision == null ? null : revision.getStatus(), revision == null ? null : revision.getRejectionReason());
    }
}

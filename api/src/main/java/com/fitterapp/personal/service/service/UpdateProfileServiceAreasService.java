...
public class UpdateProfileServiceAreasService {
    ...
    public UpdateProfileServiceAreasResult update(UpdateProfileServiceAreasCommand command) {
        ...
        List<ServiceAreaInput> areas = command.serviceAreas().stream()
                .map(this::normalize)
                .toList();
        ...
        var entities = areas.stream()
                .map(area -> RevisionServiceArea.create(revision, area.city(), area.stateCode(),
                        area.neighborhood(), area.description(), createdAt))
                .toList();
        if (!entities.isEmpty()) {
    ...
    private ServiceAreaInput normalize(ServiceAreaInput area) {
        ...
        return new ServiceAreaInput(city, stateCode, trimOptional(area.neighborhood()), trimOptional(area.description()));
    }

    private void validateNoDuplicates(List<ServiceAreaInput> areas) {
        Set<String> locations = new HashSet<>();
        for (ServiceAreaInput area : areas) {
            String key = (area.stateCode() + '|' + area.city() + '|' + area.neighborhood()).toLowerCase(Locale.ROOT);

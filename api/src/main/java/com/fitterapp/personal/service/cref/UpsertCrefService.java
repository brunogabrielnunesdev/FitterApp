...
public class UpsertCrefService {
  ...
  public UpsertCrefResult upsert(UpsertCrefCommand command) {
    ...
    Cref cref =
        crefRepository
            .findByPersonalId(profile.getId())
            .map(
                existing ->
                    update(existing, registrationCode, command.documentImageKey(), updatedAt))
            .orElseGet(
                () -> create(profile, registrationCode, command.documentImageKey(), updatedAt));
  ...
  private Cref create(
    ...
    Cref cref = Cref.pendingReview(profile, registrationCode, documentImageKey, createdAt);
    return crefRepository.save(cref);
  }

  private Cref update(
      Cref cref, String registrationCode, String documentImageKey, OffsetDateTime updatedAt) {

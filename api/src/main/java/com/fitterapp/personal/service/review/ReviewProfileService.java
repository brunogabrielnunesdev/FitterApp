...
public class ReviewProfileService {
  ...
  public ReviewProfileResult approve(ApproveProfileCommand command) {
    ...
    var revision = pending(profile.getCurrentRevision());
    var admin = users.findById(command.adminUserId()).orElseThrow();
    var personalRole =
        roles
            .findByName(RoleName.PERSONAL)
            .orElseThrow(() -> new RoleNotConfiguredException(RoleName.PERSONAL));
    OffsetDateTime now = OffsetDateTime.now(clock);
    revision.approve(admin, now);
    profile.approve(now);
    var id = new UserRoleId(profile.getUser().getId(), personalRole.getId());
    if (!userRoles.existsById(id))
      userRoles.save(UserRole.granted(profile.getUser(), personalRole, admin, now));
  ...
  public ReviewProfileResult reject(RejectProfileCommand command) {
    ...
    var admin = users.findById(command.adminUserId()).orElseThrow();
    OffsetDateTime now = OffsetDateTime.now(clock);
    revision.reject(admin, command.reason().trim(), now);
    profile.reject(now);
    return new ReviewProfileResult(profile.getId(), revision.getId());
  }

  private com.fitterapp.personal.entity.profile.ProfileRevision pending(
      com.fitterapp.personal.entity.profile.ProfileRevision revision) {
    if (revision == null || revision.getStatus() != ProfileRevisionStatus.PENDING_REVIEW)
      throw new ProfileNotPendingReviewException();
    return revision;
  }
}

...
class ModerationPersistenceTests {
  ...
  void persistsReportEvidenceAndResolution() {
    ...
    User reporter =
        activeUser(
            "Reporting Student", "moderation-reporter@fitterapp.com", "+5544999999930", createdAt);
    User reviewer =
        activeUser(
            "Moderation Admin", "moderation-reviewer@fitterapp.com", "+5544999999931", createdAt);
    Profile personal = personal(reporter, "reported-personal", createdAt);

    Report report =
        Report.againstPersonal(
            reporter,
            personal,
    ...
    ReportEvidence evidence =
        ReportEvidence.attach(
            report,
            (short) 1,
            "moderation/reports/" + report.getId() + "/" + UUID.randomUUID() + ".webp",
            createdAt);
    ...
    ReportEvidence savedEvidence = entityManager.find(ReportEvidence.class, evidence.getId());
    ...
    assertThat(savedReport.getResolution()).isEqualTo(ReportResolution.CORRECTION_REQUESTED);
    assertThat(savedReport.getPersonalProfile().getId()).isEqualTo(personal.getId());
    assertThat(savedReport.getReviewedBy().getId()).isEqualTo(reviewer.getId());
  ...
  void persistsSuspensionAndApprovedReactivationRequest() {
    ...
    User personalUser =
        activeUser(
            "Suspended Personal", "suspended-personal@fitterapp.com", "+5544999999932", createdAt);
    User admin =
        activeUser(
            "Suspension Admin", "suspension-admin@fitterapp.com", "+5544999999933", createdAt);
    Profile personal = personal(personalUser, "suspended-personal", createdAt);
    ...
    ReactivationRequest request =
        ReactivationRequest.request(
            suspension, personalUser, "As informacoes foram corrigidas", createdAt.plusDays(7));
    ...
    ReactivationRequest savedRequest =
        entityManager.find(ReactivationRequest.class, request.getId());
    ProfileSuspension savedSuspension =
        entityManager.find(ProfileSuspension.class, suspension.getId());

    assertThat(savedRequest.getStatus()).isEqualTo(ReactivationRequestStatus.APPROVED);
    assertThat(savedRequest.getSuspension().getId()).isEqualTo(suspension.getId());
  ...
  void persistsAccountBlockBlacklistAndAppeal() {
    OffsetDateTime createdAt = now();
    User blockedUser =
        activeUser("Blocked User", "blocked-user@fitterapp.com", "+5544999999934", createdAt);
    User admin =
        activeUser("Block Admin", "block-admin@fitterapp.com", "+5544999999935", createdAt);

    AccountBlock block =
        AccountBlock.block(blockedUser, null, admin, "Fraude confirmada", createdAt);
    ...
    ModerationAppeal appeal =
        ModerationAppeal.forAccountBlock(
            block, blockedUser, "Solicito uma nova analise", createdAt.plusMinutes(5));
    entityManager.persist(appeal);
    appeal.reject(admin, "Evidencias do bloqueio confirmadas", createdAt.plusMinutes(20));

    entityManager.flush();
    entityManager.clear();

    AccountBlock savedBlock = entityManager.find(AccountBlock.class, block.getId());
    BlacklistEntry savedEntry = entityManager.find(BlacklistEntry.class, entry.getId());
    ModerationAppeal savedAppeal = entityManager.find(ModerationAppeal.class, appeal.getId());
    ...
    assertThat(savedEntry.getIdentifierType()).isEqualTo(BlacklistIdentifierType.EMAIL);

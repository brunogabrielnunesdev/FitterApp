...
class AuthenticationPersistenceTests {
  ...
  void loadsUserRoleWithItsCompositeKeyAndRelationships() {
    ...
    entityManager.clear();
    UserRole userRole = entityManager.find(UserRole.class, new UserRoleId(studentId, (short) 1));
  ...
  void loadsRefreshTokenRotationAndPostgresIpAddress() {
    ...
    insertRefreshToken(
        replacementId, userId, familyId, SECOND_HASH, createdAt.plusMinutes(1), null, null);
    insertRefreshToken(
        originalId,
        userId,
        familyId,
  ...
  void loadsEmailVerificationAndPasswordResetTokens() {
    ...
    EmailVerificationToken verification =
        entityManager.find(EmailVerificationToken.class, verificationId);
    PasswordResetToken reset = entityManager.find(PasswordResetToken.class, resetId);

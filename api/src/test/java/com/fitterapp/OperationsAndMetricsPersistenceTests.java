...
class OperationsAndMetricsPersistenceTests {
    ...
    void persistsSearchAndPersonalMarketplaceEvents() {
        ...
        User student = activeUser(
                "Analytics Student",
                "analytics-student@fitterapp.com",
                "+5544999999941",
                occurredAt);
        Profile personal = personal(student, "analytics-personal", occurredAt);

        SearchEvent search = SearchEvent.record(
                student,
                EventSource.MOBILE_APP,
                "musculacao",
                objectMapper.createObjectNode()
                        .put("city", "Umuarama")
                        .put("serviceMode", "IN_PERSON"),
                8,
                occurredAt);
        ProfileViewEvent view = ProfileViewEvent.personalView(
                student,
                personal,
                EventSource.MOBILE_APP,
                "Umuarama",
                occurredAt.plusMinutes(1));
        ContactEvent contact = ContactEvent.whatsappToPersonal(
                student,
                personal,
                EventSource.MOBILE_APP,
                "Umuarama",
        ...
        ProfileViewEvent savedView = entityManager.find(
    ...
    void persistsAnonymizedAccessAndContactEvents() {
        ...
        AppAccessEvent access = AppAccessEvent.returned(
                null,
        ...
        AppAccessEvent savedAccess = entityManager.find(
                AppAccessEvent.class,
                access.getId());
        ContactEvent savedContact = entityManager.find(ContactEvent.class, contact.getId());

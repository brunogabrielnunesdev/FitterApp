...
class ProfileVersioningPersistenceTests {
    ...
    void persistsCrefReviewAndPublishedProfileRevision() {
        ...
        Profile savedProfile = entityManager.find(
                Profile.class,
                profile.getId());
        ProfileRevision savedRevision = entityManager.find(
                ProfileRevision.class,
                revision.getId());
        ...
        assertThat(savedRevision.getStatus())
                .isEqualTo(ProfileRevisionStatus.APPROVED);
    ...
    void persistsVersionedModalitiesServiceModesAndAreas() {
        ...
        RevisionModality revisionModality =
                RevisionModality.link(revision, modality);
        RevisionServiceMode serviceMode =
                RevisionServiceMode.of(revision, ServiceMode.IN_PERSON);
        RevisionServiceArea serviceArea = RevisionServiceArea.create(
                revision,
                "Umuarama",
                "PR",
                "Zona I",
        ...
        RevisionModality savedModality = entityManager.find(
                RevisionModality.class,
                new RevisionModalityId(revision.getId(), (short) 1));
        RevisionServiceMode savedMode = entityManager.find(
                RevisionServiceMode.class,
                new RevisionServiceModeId(revision.getId(), ServiceMode.IN_PERSON));
        RevisionServiceArea savedArea = entityManager.find(
                RevisionServiceArea.class,
                serviceArea.getId());

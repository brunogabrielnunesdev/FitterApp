...
import com.fitterapp.personal.dto.review.*;
...
import com.fitterapp.personal.service.review.*;
...
class AdminProfileControllerTests {
  @Mock ReviewProfileService review;
  @Mock ListProfilesForReviewService pending;

  @Test
  void rejectsUsingAdminJwtSubject() {
    UUID admin = UUID.randomUUID(), profile = UUID.randomUUID(), revision = UUID.randomUUID();
    when(review.reject(any())).thenReturn(new ReviewProfileResult(profile, revision));
    var response =
        new AdminProfileController(review, pending, new ProfileMapper())
            .reject(jwt(admin), profile, new RejectProfileRequestDto("CREF inválido"));

...
class AcademyCatalogPersistenceTests {
  ...
  void persistsReviewedAndPublishedAcademyRevision() {
    ...
    User applicant =
        activeUser(
            "Academy Applicant", "academy-applicant@fitterapp.com", "+5544999999920", createdAt);
    User reviewer =
        activeUser("Platform Owner", "academy-reviewer@fitterapp.com", "+5544999999921", createdAt);

    AcademyProfile academy = AcademyProfile.draft("studio_performance", createdAt);
    entityManager.persist(academy);
    entityManager.flush();

    AcademyCnpj cnpj = AcademyCnpj.pendingReview(academy, "12345678000190", createdAt);
    entityManager.persist(cnpj);

    AcademyProfileRevision revision =
        AcademyProfileRevision.draft(academy, 1, applicant, true, createdAt);
    revision.updatePublicData(
        "Studio Performance",
        "Treinamento próximo e individualizado.",
        "+5544999999920",
        "@studio.performance",
        "academies/" + academy.getId() + "/" + java.util.UUID.randomUUID() + ".webp",
        createdAt);
    revision.updateAddress(
        "87501000", "Avenida Paraná", "1200", "Sala 2", "Zona I", "Umuarama", "PR", createdAt);
    ...
    AcademyProfile savedAcademy = entityManager.find(AcademyProfile.class, academy.getId());
    AcademyProfileRevision savedRevision =
        entityManager.find(AcademyProfileRevision.class, revision.getId());
    AcademyCnpj savedCnpj = entityManager.find(AcademyCnpj.class, cnpj.getId());

    assertThat(savedAcademy.getStatus()).isEqualTo(AcademyProfileStatus.PUBLISHED);
    assertThat(savedAcademy.getCurrentRevision().getId()).isEqualTo(revision.getId());
    assertThat(savedAcademy.getPublishedRevision().getId()).isEqualTo(revision.getId());
    assertThat(savedRevision.getStatus()).isEqualTo(AcademyProfileRevisionStatus.APPROVED);
    ...
    assertThat(savedRevision.getReviewedBy().getId()).isEqualTo(reviewer.getId());
  ...
  void persistsAcademyOwnerAndVersionedModality() {
    OffsetDateTime createdAt = now();
    User owner =
        activeUser("Academy Owner", "member-owner@fitterapp.com", "+5544999999922", createdAt);
    AcademyProfile academy = AcademyProfile.draft("movimento_academia", createdAt);
    entityManager.persist(academy);
    entityManager.flush();

    AcademyMember member = AcademyMember.active(academy, owner, AcademyMemberRole.OWNER, createdAt);
    AcademyProfileRevision revision =
        AcademyProfileRevision.draft(academy, 1, owner, true, createdAt);
    entityManager.persist(member);
    entityManager.persist(revision);
    entityManager.flush();

    Modality modality = entityManager.find(Modality.class, (short) 1);
    AcademyRevisionModality revisionModality = AcademyRevisionModality.link(revision, modality);
    entityManager.persist(revisionModality);
    entityManager.flush();
    entityManager.clear();

    AcademyMember savedMember =
        entityManager.find(
            AcademyMember.class, new AcademyMemberId(academy.getId(), owner.getId()));
    AcademyRevisionModality savedModality =
        entityManager.find(
  ...
  void persistsBilateralPartnershipLifecycle() {
    ...
    AcademyProfile academy = AcademyProfile.draft("parceria_fitness", createdAt);
    Profile personal = Profile.draft("Partnership Personal", "partnership-personal", createdAt);
    ...
    AcademyPersonalPartnership partnership =
        AcademyPersonalPartnership.request(
            academy, personal, PartnershipInitiator.ACADEMY, academyOwner, createdAt);
    ...
    AcademyPersonalPartnership saved =
        entityManager.find(AcademyPersonalPartnership.class, partnership.getId());

    assertThat(saved.getStatus()).isEqualTo(PartnershipStatus.ACCEPTED);
    assertThat(saved.getInitiatedBy()).isEqualTo(PartnershipInitiator.ACADEMY);
